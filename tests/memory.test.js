import { CONTEXT_COMPOSER } from "../src/engine/context.js";
...
for (const composer of [MEMORY_COMPOSER, CONTEXT_COMPOSER]) {
  ...
  test(`${composer}: branch memory stays bounded, avoids recent choices and leaves siblings independent`, async () => {
  ...
  test(`${composer}: a tiny corpus reports a revisit when no fresh shortlist alternative remains`, async () => {
    const entries = [];
    await runAutomatic({
      ...options(fixture(1)),
      maxEpochs: 3,
  ...
  test(`${composer}: memory archives retain replay inputs and reject malformed or inconsistent ledgers`, async () => {
    const engine = fixture();
    let entry;
    await runAutomatic({
      ...options(engine),
  ...
  test(`${composer}: interruption preserves the same memory and future as an uninterrupted lineage`, async () => {
    const engine = fixture(),
      expected = [],
      actual = [];
    await runAutomatic({
    ...
    let checkpoint = createAutomaticState(137, composer);
