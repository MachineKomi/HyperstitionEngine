...
## Second pass: cuts, clocks and persistence

The publisher's *Circuitries* excerpt makes cinema, temporal rearrangement and cybernetic machinery part of its conceptual movement. My design inference is that a cut needs a persistent frame: a reader should perceive the relation changing, rather than chase a panel that has moved. The instrument now reserves the map's generation slots and gives text a fixed reading surface. These are interface decisions inspired by the reading, not claims that Land proposed this interface. [Urbanomic, Circuitries](https://www.urbanomic.com/chapter/fanged-noumena-circuitries/).

*Hypervirus* uses viral and media language to connect propagation, replication and transformations of a host. Read here as speculative cultural theory, it suggests persistence and altered relations as richer artistic materials than constant visual escalation. The practical experiment is a variable clock with a distinct reading beat: a trace can linger while its consequences remain available to inspect. The fictional vocabulary supplies atmosphere; the engine retains explicit operator controls and performs no propagation outside its local writing process. [Urbanomic, Hypervirus](https://www.urbanomic.com/wp-content/uploads/2015/03/hypervirus.pdf).

At 1×, each epoch uses 0.8 seconds of charge, 1.5 seconds per generation, and eight seconds of reading. These values are an initial design judgment, not a measured threshold of human comprehension. The same seed produces the same heirs under different pacing. Reader preference and comprehension still need evaluation; increasing clock speed cannot substitute for improving composition.
