...
**v0.22 checkpoint:** the [seed foundry](SEED_FOUNDRY.md) randomizes startup seeds and all five input controls. Three dials and two switches contribute explicit values to each new draw. Editing controls does not interrupt a run; drawing starts a new one. Numeric-seed replay remains unchanged. Next: observe whether users distinguish pending control edits, random drawing and exact-seed restarting before adding further controls.
